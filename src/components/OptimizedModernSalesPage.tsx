...
const OptimizedModernSalesPage = () => {
  ...
  const handleCheckoutComplete = useCallback(() => {
    ...
    refetchCustomers();
    refetchProducts(); // Use regular refetch to get updated data from cache/server
    clearCart();
  ...
  // Listen for product stock updates and refresh products immediately
  useEffect(() => {
    const handleProductUpdate = (event: CustomEvent) => {
      console.log('[OptimizedModernSalesPage] Product updated locally event received:', event.detail);
      // Use regular refetch for both online and offline - it will use local cache when offline
      refetchProducts();
    };

    const handleSaleComplete = () => {
      console.log('[OptimizedModernSalesPage] Sale completed event received');
      // Refresh products to show updated stock from local cache
      refetchProducts();
    };
    ...
    window.addEventListener('product-updated-locally', handleProductUpdate as EventListener);
    window.addEventListener('product-updated-server', handleProductUpdate as EventListener);
    window.addEventListener('sale-completed', handleSaleComplete);
    window.addEventListener('checkout-completed', handleCheckoutComplete);
    
    return () => {
      window.removeEventListener('product-updated-locally', handleProductUpdate as EventListener);
      window.removeEventListener('product-updated-server', handleProductUpdate as EventListener);
  ...
  // Simple search handler for the new fixed bottom search
  ...
  const SearchPanel = () => (
    <div className="flex flex-col h-full relative">
      ...
      {/* Mobile Category Filter - Show above products on mobile */}
      {isMobile && (
        <div className="mb-4">
          <Select value={selectedCategory} onValueChange={setSelectedCategory}>
            <SelectTrigger className="w-full bg-muted border-0 rounded-xl py-3 focus:ring-2 focus:ring-ring">
              <SelectValue placeholder="All Categories" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Categories</SelectItem>
      ...
      <div 
        ref={productListRef} 
        className={`flex-1 overflow-y-auto ${isMobile ? 'pb-20' : 'pb-8'}`}
        style={{
          // On mobile, ensure there's space for the fixed bottom search
          paddingBottom: isMobile ? '80px' : undefined
        }}
      >
        <div className={`grid ${isMobile ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'} gap-6`}>
