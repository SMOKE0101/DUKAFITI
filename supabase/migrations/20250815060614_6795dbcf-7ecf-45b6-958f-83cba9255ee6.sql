...
-- This is the core issue - no triggers exist on the sales table currently
...
-- Also add a comprehensive test sale to validate the trigger works
-- This will be a test record that can be deleted after validation
INSERT INTO public.sales (
  ...
  'test-trigger-validation-' || gen_random_uuid()::text,
  '00000000-0000-0000-0000-000000000000'::uuid, -- test user ID
  'test-product-' || gen_random_uuid()::text,
  'Test Product for Trigger Validation',
  1,
  100.00,
  50.00,
  50.00,
  ...
  'Test Customer',
  'split',
  '{"cashAmount": 50, "mpesaAmount": 0, "debtAmount": 50, "discountAmount": 0}'::jsonb,
  now(),
  true
) 
-- Only insert if no existing test record to avoid duplicates
WHERE NOT EXISTS (
  SELECT 1 FROM public.sales 
  WHERE product_name = 'Test Product for Trigger Validation' 
